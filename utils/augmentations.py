...
# torch.fx.wrap("torch.from_numpy") 

class ImageTransform(torch.nn.Module):
    def forward(self, im):
        nparray = torch.as_tensor(im)
        nparray = nparray.permute(2, 0, 1)
        return nparray
...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    new_unpad = (640, 360) # TODO HL int(round(shape[1] * r)), int(round(shape[0] * r))  (640, 360)
    dw, dh = new_shape[1] - new_unpad[0], new_shape[0] - new_unpad[1]  # wh padding  0, 24
    if auto:  # minimum rectangle  # False
    ...
    dw /= 2  # divide padding into 2 sides  # 0.0
    dh /= 2  # 12.0

    if True: # TODO HL shape[::-1] != new_unpad:  # resize  # True
        # torchtensor = torch.as_tensor(im)
        # torchtensor = torchtensor.permute(2, 0, 1)
        # im = resize_with_torch(torchtensor, (new_unpad[1], new_unpad[0])) # TODO HL changed that
        # im = np.asarray(im)
        im = cv2.resize(im, new_unpad, interpolation=cv2.INTER_LINEAR)
        # np.testing.assert_almost_equal(im, im_old)
    top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1)) # (12,12)
    left, right = int(round(dw - 0.1)), int(round(dw + 0.1)) # (0,0)
    im = cv2.copyMakeBorder(im, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # TODO!!!!! (just commended out) add border
    return im, ratio, (dw, dh)



def resize_with_torch(im, new_unpad):
    # Convert NumPy array to PyTorch tensor
    im_tensor = ImageTransform()(im)
    # im_tensor = torch.from_numpy(im).permute(2, 0, 1)  # Convert HWC to CHW

    # Resize using PyTorch
    im_resized = TF.resize(im_tensor, new_unpad, interpolation=TF.InterpolationMode.BILINEAR)

    # Convert back to NumPy array
    im_resized = im_resized.permute(1, 2, 0).numpy()  # Convert CHW back to HWC
